...
public class Balancer3 {
	...
	public static boolean isBalanced(String input) {
		...
		*/
		...
		char closed3 = ']';
		char closed4 = '>';
		ArrayList<Character> balancer = new ArrayList<Character>(); //An arrayList of characters
	...
	int p=0;
	while(balancer.size() >0){  //if there is nesting, it keeps checking
		...
		if(balancer.size() >0){
				if(balancer.get(p) == open2 && balancer.get(p+1) == closed2){
					balancer.remove(p);
					balancer.remove(p);   //after removing once, the ArrayList shifts to the left
				}
		}
		if(balancer.size() >0){
